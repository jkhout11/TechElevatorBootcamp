...
-- optional using table aliases only if desired
...
SELECT 
	state.state_name, 
	MIN(park.date_established) AS earliest_park
FROM state 
...
SELECT city.city_name AS place_name, 'City' AS kind_of_place
FROM city
WHERE city.city_name LIKE 'W%'
UNION
SELECT state.state_name AS place_name, 'State' AS kind_of_place
FROM state
WHERE state.state_name LIKE 'W%'
ORDER BY kind_of_place;
