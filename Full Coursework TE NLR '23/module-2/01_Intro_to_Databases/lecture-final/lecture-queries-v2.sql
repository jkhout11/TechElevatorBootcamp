...
FROM
	city
WHERE
	city_name LIKE 'A%';  -- ILIKE --for case insensitive
...
-- _ wildcard (extended material)
SELECT 
	city_name
FROM
	city
WHERE
	city_name LIKE '_a%'  -- finds any values that have a in the second position
...
FROM
	city
WHERE
	city_name ILIKE 'c%e'  -- finds any values that start with "c" and ends with "e"
...
FROM 
	state
WHERE 
	sales_tax >= 7 OR population < 1000000;	
