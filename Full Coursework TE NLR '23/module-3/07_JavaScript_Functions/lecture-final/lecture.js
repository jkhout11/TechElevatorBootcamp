 ...
 * 
 * @param {string} name the name of the person we're dercribing
 * @param {number} age the age of the person
 * @param {string[]} listOfQuirks a list of funny quirks that we capture
 * @param {string} separator the full descriptive string
...
//Additional resource - array functions, etc.
//https://developer.mozilla.org/en-US/docs/Learn/JavaScript


function displayNames(){
  const arrayNames = ["Neo", "Trinity", "Morpheus"];

  arrayNames.forEach((element) => {
    console.log(`Welcome to the real ${element}`)
  });
}


function multiplyAllByTwo(arrayNums){ 
...
function multiplyAllByTwoPrint(arrayNums){ 
  const mapNums = arrayNums.map(num => num * 2); //short hand
  console.log(mapNums);
}
...
function createGreeting(names){
  return names.map((name)=> {
    return 'Welcome to the real ' + name;
  });
