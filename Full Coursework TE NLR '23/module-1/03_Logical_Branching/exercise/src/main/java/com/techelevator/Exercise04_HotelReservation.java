...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if (numberOfNights <= 2)
            stayTotal = DAILY_RATE * numberOfNights;
        else if (numberOfNights >= 3)
            stayTotal = DISCOUNT_RATE * numberOfNights;
        return stayTotal;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        ...
        if (numOfTotalNights <= 2)
            stayTotal = DAILY_RATE * numOfTotalNights;
        else if (numOfTotalNights >= 3)
            stayTotal = DISCOUNT_RATE * numOfTotalNights;
        if (includesParking)
            stayTotal = (stayTotal) + (numOfTotalNights * PARKING_RATE);
        return stayTotal;
